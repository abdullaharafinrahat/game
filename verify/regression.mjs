...
async function tryMove(label, moveX, moveY, yaw) {
  ...
  const after = await page.evaluate(() => {
    const g = window.__game;
    return { x: g.player.position.x, z: g.player.position.z, yaw: g.player.root.rotation.y };
  });
  ...
  return { label, moved: len, dotView, dotRight, dotModel, yaw: after.yaw, view: before.view, dir: unit };
}
...
check('model faces direction of travel', fwdYaw.dotModel > 0.9, `dot(travel, model forward) = ${fwdYaw.dotModel.toFixed(3)}`);
